<template>
  <div class="p-8 max-w-7xl mx-auto w-full h-full flex flex-col transition-colors duration-300">
    <div class="flex justify-between items-end mb-8">
      <div>
        <h1 class="text-4xl font-black mb-2 text-zinc-900 dark:text-white">The Master Vault</h1>
        <p class="text-zinc-500 dark:text-zinc-400">
          View, filter, and export your extracted leads.
          <span v-if="!leadStore.isLoading" class="text-electric-blue dark:text-cyber-lime font-bold ml-2">{{ leadStore.leadCount }} total</span>
        </p>
      </div>
      <button @click="exportCSV" :disabled="filteredLeads.length === 0"
        class="px-6 py-3 bg-white dark:bg-zinc-800 hover:bg-zinc-50 dark:hover:bg-zinc-700 text-zinc-900 dark:text-white rounded-xl font-bold transition-all border border-zinc-200 dark:border-zinc-700 flex items-center gap-2 shadow-sm cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed">
    ...
    <div class="flex flex-col md:flex-row gap-4 mb-6">
      ...
      <div class="flex gap-2 bg-white dark:bg-zinc-900 p-1.5 rounded-xl border border-zinc-200 dark:border-zinc-800 shadow-sm">
    ...
    <div v-else class="flex-1 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-3xl overflow-hidden flex flex-col shadow-sm">
      <div class="overflow-x-auto">
        <table class="w-full text-left text-sm text-zinc-600 dark:text-zinc-400">
          <thead class="text-xs uppercase bg-zinc-50 dark:bg-zinc-950/50 text-zinc-500 sticky top-0 border-b border-zinc-200 dark:border-zinc-800/50">
            <tr>
              <th class="px-6 py-4 font-bold tracking-widest">Company</th>
              <th class="px-6 py-4 font-bold tracking-widest">Contact</th>
              <th class="px-6 py-4 font-bold tracking-widest">Website</th>
